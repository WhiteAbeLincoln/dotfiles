...
- **Secret management — switch to sops-nix in Phase 3 (decided, no spec yet).** Sealed-secrets (Phases 0–2) has two drawbacks the operator wants gone: the truth lives in two places (plaintext in git-crypt'd `secrets/globalhawk.nix` **and** the sealed blob in `k8s/apps/torrent.nix`), and the decryption key is **cluster-generated ephemeral state** — a from-scratch cluster rebuild loses it, so every `SealedSecret` must be re-sealed or the key restored. The chosen replacement is **sops-nix rendering the k8s `Secret` at activation time** (the `rorosen/k3s-nix` pattern the spec already cited as the reversible alternative): `sops.templates` decrypts, on the host using the **host key** (SSH `ed25519` via ssh-to-age, or a dedicated age key), directly into `/var/lib/rancher/k3s/server/manifests/<name>.yaml` (root 0600, **never the world-readable store**); k3s auto-applies it. This gives (a) a **single source of truth** — the sops-encrypted file is the Secret's only source, no sealed blob; (b) **no ephemeral cluster state** — the Secret is re-derived from source on every activation, so a rebuild just needs the host key provisioned (the same bootstrap git-crypt already needs), nothing to re-seal or back up. Phase 3 is the natural moment because it adds immich's DB-password secret anyway; that pass **migrates all secrets** (mullvad WG, immich, restic, wireguard) to sops-nix and **removes the sealed-secrets controller** from `k3s.nix`. sops-nix is preferred over agenix specifically for `sops.templates` (manifest rendering) and GPG+age support. Reversible, contained swap: replace the `SealedSecret` block with a `sops.templates`-rendered Secret and drop the controller.
