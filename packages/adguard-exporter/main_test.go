...
func TestMetricsExcludeQueryDetails(t *testing.T) {
	api := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch r.URL.Path {
		case "/control/stats":
			_, _ = io.WriteString(w, `{"num_dns_queries":12,"num_blocked_filtering":5,"avg_processing_time":0.003,"top_clients":[{"private-client":10}],"top_queried_domains":[{"private.example":8}]}`)
		case "/control/status":
			_, _ = io.WriteString(w, `{"protection_enabled":true}`)
		case "/control/filtering/status":
			_, _ = io.WriteString(w, `{"enabled":true,"filters":[{"name":"private-list"}]}`)
		default:
			http.NotFound(w, r)
		}
	}))
...
func TestMetricsRejectMissingRequiredField(t *testing.T) {
	api := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch r.URL.Path {
		case "/control/stats":
			_, _ = io.WriteString(w, `{"num_dns_queries":12,"avg_processing_time":0.003}`)
		case "/control/status":
			_, _ = io.WriteString(w, `{"protection_enabled":true}`)
		case "/control/filtering/status":
