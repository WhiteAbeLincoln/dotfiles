# A copy of https://github.com/nix-darwin/nix-darwin/blob/nix-darwin-25.05/modules/nix/linux-builder.nix
# changed to work with determinate-nix.
# Changes:
# - module options are now under `determinate-nix` instead of `nix`
# - `nix.enabled` assertion has been removed
{
  config,
  lib,
  pkgs,
...
with lib; let
  cfg = config.determinate-nix.linux-builder;
in {
  options.determinate-nix.linux-builder = {
  ...
  config = mkMerge [
    ...
    (mkIf cfg.enable {
      ...
      determinate-nix.customSettings.builders-use-substitutes = true;
    })
  ];
}
