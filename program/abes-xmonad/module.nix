...
in {
  ...
  config = mkIf cfg.enable (
    let
      ...
      mapKeybindings = mapAttrsToList (key: value: {
        inherit key;
      ...
      mapConfig = opts: let
        ...
        termHandled =
          if filtered ? terminal
          then
            filtered
            // {
              terminal = {
                key = filtered.terminal.keyMap;
                value = filtered.terminal.value;
                name = filtered.terminal.value;
              };
            }
          else filtered;
        final =
          if termHandled ? keybindings
          then termHandled // {keybindings = mapKeybindings (filtered.keybindings);}
          else termHandled;
      in
        final;
      abesXmonadConfig =
        if cfg.options != null
        then mapConfig cfg.options
        else cfg.options;
